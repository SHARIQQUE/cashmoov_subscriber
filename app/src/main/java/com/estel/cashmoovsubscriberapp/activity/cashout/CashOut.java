...
public class CashOut extends LogoutAppCompactActivity implements View.OnClickListener {
    public static CashOut cashoutC;
    ...
    public void qrScan(View v){
        Intent i = new Intent(cashoutC, QrCodeActivity.class);
            ...
            public boolean onTouch(View v, MotionEvent event) {
                ...
                if(event.getAction() == MotionEvent.ACTION_UP) {
                    if(event.getRawX() >= (etRecipientNo.getRight() - etRecipientNo.getCompoundDrawables()[DRAWABLE_RIGHT].getBounds().width())) {
                        ...
                        Intent intent = new Intent(CashOut.this,
...
//                        tvRate.setText("");
    ...
    private void setOnCLickListener() {
        tvSend.setOnClickListener(cashoutC);
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode != Activity.RESULT_OK) {
            ...
            if (result != null) {
                AlertDialog alertDialog = new AlertDialog.Builder(cashoutC).create();
...
*/
        ...
        switch (view.getId()) {
            case R.id.tvSend:
                ...
                if(etRecipientNo.getText().toString().trim().length()<9){
                    MyApplication.showErrorToast(cashoutC,getString(R.string.val_inst_agemt_branch_no_len));
                    return;
                }
                if(etAmount.getText().toString().trim().replace(",","").isEmpty()){
                    MyApplication.showErrorToast(cashoutC,getString(R.string.val_amount));
                    return;
                }
                ...
                if(Double.parseDouble(etAmount.getText().toString().trim().replace(",",""))>MyApplication.CashOutMaxAmount) {
                    MyApplication.showErrorToast(cashoutC,getString(R.string.val_amount_max)+" "+MyApplication.CashOutMaxAmount);
                    return;
                }

                if(isSuccess){
                try{
                    dataToSend.put("transactionType","100001");
                    ...
                    MyApplication.saveString("AMOUNTCASHOUT",etAmount.getText().toString().trim().replace(",",""),cashoutC);
                    System.out.println("Data Send "+dataToSend.toString());
                    Intent i=new Intent(cashoutC, CashOutConfirmScreen.class);
                    startActivity(i);
                }catch (Exception e){
                    ...
                    etAmount.setText("");
                    MyApplication.showErrorToast(cashoutC,getString(R.string.val_valid_inst_agent_branch_no));
                }
    ...
    public void callwalletOwnerDetailsQR(String Code){
        API.GET("ewallet/api/v1/walletOwner/"+Code, new Api_Responce_Handler() {
            ...
            public void failure(String aFalse) {
                MyApplication.showToast(cashoutC,aFalse);
            }
        });
    ...
    public void callserviceCategory(){
        API.GET("ewallet/api/v1/serviceProvider/serviceCategory?serviceCode=100003&serviceCategoryCode=100012&status=Y", new Api_Responce_Handler() {
            @Override
            public void success(JSONObject jsonObject) {
                MyApplication.hideLoader();
                if(jsonObject.optString("resultCode").equalsIgnoreCase("0")){
    ...
    private void callApiAmountDetails() {
        try {
            ...
            API.GET("ewallet/api/v1/exchangeRate/getAmountDetails?"+"sendCurrencyCode="+fromCurrencyCode+
                            ...
                            "&walletOwnerCode="+MyApplication.getSaveString("walletOwnerCode", cashoutC)+
                            "&remitAgentCode="+MyApplication.getSaveString("walletOwnerCode", cashoutC)+
...
//                                    }
                                    ...
                                    tvSend.setVisibility(View.GONE);
                                    MyApplication.showToast(cashoutC,jsonObject.optString("resultDescription", "N/A"));
                                }
                            }
                        }
    ...
    private void formatInput(EditText editText,CharSequence s, int start, int count) {
        if(MyApplication.checkMinMax(cashoutC,s,editText
